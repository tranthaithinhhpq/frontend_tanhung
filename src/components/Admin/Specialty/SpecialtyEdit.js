...
const SpecialtyEdit = () => {
    ...
    const fetchData = async () => {
        ...
        if (current) {
            setForm({
                ...current,
                isSelectable: current.isSelectable ?? true, // ✅ load từ DB, fallback true
            });
            setPreview(current.image ? `${BACKEND_URL}${current.image}` : '');
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        data.append('markdownContent', form.markdownContent);
        data.append('displayOrder', form.displayOrder);
        data.append('isSelectable', String(form.isSelectable)); // ✅ gửi boolean
        if (image) data.append('image', image);
            ...
            <Row>
                <Col md={previewMode ? 6 : 12}>
                    <Form onSubmit={handleSubmit}>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>Ảnh hiện tại / preview ảnh mới</Form.Label>
                            <br />
                            {preview && (
                                <Image
                                    src={preview}
                        ...
                        <Form.Group className="mb-3">
                            <Form.Check
                                ...
                                onChange={(e) =>
                                    setForm({ ...form, isSelectable: e.target.checked })
                                }
                            />
                        </Form.Group>
