...
const SpecialtyCreate = () => {
    const [form, setForm] = useState({
        ...
        description: '',
        markdownContent: '',
        isSelectable: true, // ✅ mặc định cho phép đặt lịch
    });
    const [image, setImage] = useState(null);
    const [previewImg, setPreviewImg] = useState('');
    const [previewMode, setPreviewMode] = useState(false);
    ...
    const handleSubmit = async (e) => {
        ...
        data.append('description', form.description);
        data.append('markdownContent', form.markdownContent);
        data.append('isSelectable', String(form.isSelectable)); // ✅ gửi dưới dạng "true"/"false"
                        ...
                        <Card className="h-100 overflow-auto mt-2">
                            <Card.Body>
                                ...
                                <p>
                                    ...
                                    {form.isSelectable ? <span className="badge bg-success">Có</span> : <span className="badge bg-danger">Không</span>}
                                </p>
                                {previewImg && (
                                    <img
                                        src={previewImg}
