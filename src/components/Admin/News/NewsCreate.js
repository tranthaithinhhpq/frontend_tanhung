...
//                     <div className="mt-3">
//                         <Button variant="secondary" className="me-2" onClick={() => setPreviewMode(!previewMode)}>{previewMode ? "Ẩn Preview" : "Fast Preview"}</Button>
...
const NewsCreate = ({ editData }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = editData
                ? await axios.put(`/api/v1/edit/${editData.id}`, formData)
                : await axios.post("/api/v1/news/create", formData);
